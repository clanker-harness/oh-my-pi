...
import { AgentRegistry, TEAM_ADDRESS_PREFIX } from "../registry/agent-registry";
...
export class AgentProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl, context?: ResolveContext): Promise<InternalResource> {
		...
		if (outputId.startsWith(TEAM_ADDRESS_PREFIX))
			throw new Error(`agent://${outputId} is write-only; use it to broadcast to that team.`);
		if (!outputId) {
