...
Broadcast to the whole team via `write` with `path: "agent://team:{{ircTeam}}"`; that reaches every member and no one else. Address one member by its id. Your teammates are listed in full above and are NOT subject to the peer cap below.
{{/if}}
