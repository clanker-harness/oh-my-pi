...
export async function executeSend(
	...
	// scoped to one peer group so a member can talk to its own team without
	// spraying every agent in the process.
	const teamTarget = to.startsWith(TEAM_ADDRESS_PREFIX) ? to.slice(TEAM_ADDRESS_PREFIX.length).trim() : undefined;
	if (teamTarget === "")
		return coordinationErrorResult("`team:` needs a team name, e.g. agent://team:refactor.", {
			op: "send",
			from: senderId,
			to,
		});
	...
	if (isBroadcast) {
		text =
			targets.length === 0
				? teamTarget !== undefined
					? `No live members in team "${teamTarget}".`
