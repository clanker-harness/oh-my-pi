...
describe("structured subagent primitive", () => {
	...
	it("rejects a read-only agent as a team member", async () => {
		// Messaging is a `write` to `agent://…`, which a read-only agent never has,
		// so a team roster would advertise a broadcast it cannot send.
		mockDiscovery({ ...AGENT, tools: ["read", "grep"] });
		await expect(resolveEffectiveSubagentPolicy(request({ team: "refactor" }))).rejects.toThrow(
			/read-only and cannot `write` messages/,
		);
	});
