...
function registerPeer(id: string, status: "running" | "idle" = "running", team?: string): void {
...
describe("write agent:// messaging", () => {
	...
	it("broadcasts to one team only, never to the sender or non-members", async () => {
		registerPeer("Backend", "running", "core");
		registerPeer("Frontend", "idle", "core");
		registerPeer("Outsider");
		const result = await new WriteTool(makeSession({ senderId: "Backend" })).execute("team", {
			path: "agent://team:core",
			content: "schema changed",
		});
		expect(result.content[0]).toMatchObject({ text: expect.stringContaining("1 of 1 peer(s)") });
		expect(received.get("Frontend")?.map(message => [message.from, message.body])).toEqual([
			["Backend", "schema changed"],
		]);
		expect(received.get("Outsider")).toEqual([]);
		expect(received.get("Backend")).toEqual([]);

		const empty = await new WriteTool(makeSession({ senderId: "Backend" })).execute("team", {
			path: "agent://team:ghost",
			content: "anyone?",
		});
		expect(empty.content[0]).toMatchObject({ text: 'No live members in team "ghost".' });
		await expect(new AgentProtocolHandler().resolve(parseInternalUrl("agent://team:core"))).rejects.toThrow(
			"write-only",
		);
	});
