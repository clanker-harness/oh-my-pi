...
| `team` | `string` | No | Batch only; present when `task.teams.enabled=true`. Groups every item of the call into one addressable peer group. Members see each other in full on their system-prompt roster (exempt from the peer cap) and can broadcast by writing to `agent://team:<name>`. Recorded on each member's `AgentRef.team`. Read-only agents are rejected at preflight: they have no `write` tool, so such a member could not message the team. |
