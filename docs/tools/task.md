...
| `model` | `string` | No | Present only with `task.enableModelSelection=true` and plan mode disabled. Run this spawn on a specific model. Accepts a model role alias (`@smol`, `@slow`, any configured role), a selector the user authorized this session with a `^model` mention, or an entry in `task.allowedSpawnModels`; a `:level` thinking suffix and comma-separated fallback chains are allowed, and every entry in a chain must authorize. Anything else is rejected at preflight (`Cannot spawn on that model: …`) with the legal set named, so the model can self-correct. Omitting it keeps the existing precedence: `task.agentModelOverrides[agent]` > agent frontmatter `model:` > the parent's active model. Item field in batch shape, top-level in flat shape. |
