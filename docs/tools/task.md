...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, effort?, model?, role?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.enabled` is true **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off); `model` exists only when `task.enableModelSelection=true` (default on) and plan mode is disabled; `role` (plus the batch-level `team`) exists only when `task.teams.enabled=true` (default on).
...
| `model` | `string` | No | Present only with `task.enableModelSelection=true` (default on) and plan mode disabled. Run this spawn on a specific model: a `provider/id` selector or a model role alias (`@smol`, `@slow`, any configured role); a `:level` thinking suffix and comma-separated fallback chains are allowed. A selector matching no available model is rejected at preflight (`no available model matches "…"`) rather than silently falling back to the session default — the failure mode that got per-call model removed in #6438. `task.allowedSpawnModels`, when non-empty, restricts the accepted set. Omitting `model` keeps the existing precedence: `task.agentModelOverrides[agent]` > agent frontmatter `model:` > the parent's active model. Item field in batch shape, top-level in flat shape. |
| `team` | `string` | No | Batch only; present when `task.teams.enabled=true`. Groups every item of the call into one addressable peer group. Members see each other in full on their system-prompt roster (exempt from the peer cap) and can broadcast with `hub` op:`send` to:`team:<name>`. Recorded on each member's `AgentRef.team`. Read-only agents are rejected at preflight: `isReadOnlyAgent` withholds `hub`, so such a member could not act on the roster. |
| `role` | `string` | No | Present when `task.teams.enabled=true`. Short label for this member's job on the team, shown on every teammate's roster and recorded on `AgentRef.role`. |
